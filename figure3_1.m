...
% figure3_1.m
% ==============================================================================
% Purpose: Reproduce Figure 3-1 from Liddy et al. (2026, eNeuro).
%          Bayesian analysis of RSVP accuracy for the ST group across
...
%   - statistics/results/posterior_rsvp_st.xlsx
%
%   Posterior file is a pre-generated output of rsvp_binomial_st.R.
%   Re-run that script to regenerate it.
%
% Output: figures/figure3_1.tif
% ==============================================================================
...
% Create figure
f = figure("Color", "w", "Units", "Inches", "OuterPosition", [3 2 6.5 4.5]);
theme(f, "light");

n = height(DataTable);

% Color list 
groupColor = [hex2rgb("#3182bd")];
...
% Onset
swarmchart(onset_x, delta_onset, 12.5, "XJitter","density", "MarkerFaceColor", groupColor, "MarkerFaceAlpha", 0.7, "MarkerEdgeColor", "none", "LineWidth", 1);
...
% Early
swarmchart(early_x, delta_early, 12.5, "XJitter","density", "MarkerFaceColor", groupColor, "MarkerFaceAlpha", 0.7, "MarkerEdgeColor", "none", "LineWidth", 1);
boxchart(4.5*ones(size(delta_early)), delta_early, 'BoxFaceColor', groupColor, 'WhiskerLineColor', 'k', 'BoxMedianLineColor', 'k', ...
    'BoxFaceAlpha', 0.3, 'LineWidth', 1.5, 'MarkerStyle', '.', 'MarkerColor', groupColor, 'MarkerSize', 10);

% Early
swarmchart(late_x, delta_late, 12.5, "XJitter","density", "MarkerFaceColor", groupColor, "MarkerFaceAlpha", 0.7, "MarkerEdgeColor", "none", "LineWidth", 1);
...
post_draws = readtable(fullfile("statistics","results","posterior_rsvp_st.xlsx"), 'Sheet', 'PhaseAccuracy');
...
post_draws = readtable(fullfile("statistics","results","posterior_rsvp_st.xlsx"), 'Sheet', 'DeltaFromBase');
...
b.BaseLine.Color = hex2rgb('#666A6D');
...
post_draws = readtable(fullfile("statistics","results","posterior_rsvp_st.xlsx"), 'Sheet', 'OddsRatioFromBase');
post_draws.phase = categorical(post_draws.phase,{'onset','early','late'});
...
title("posterior","FontSize", 8, "Position", [2 2.5])
axis square;
...
% Save figure
exportgraphics(f, fullfile("figures", "figure3_1.tif"), 'Resolution', 600);
